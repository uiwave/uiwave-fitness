...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    const result = await post<SignInResponse>('/api/auth/sign-in/email', { email, password })
    // session.token es el token Bearer; tolera respuestas con token directo
    const token = result.session?.token ?? (result as SignInResponse & { token?: string }).token
    if (!token) throw new Error('La respuesta del servidor no incluyó un token de sesión')
    setToken(token)
    const { data } = await get<Envelope<User>>('/auth/me')
    setUser(data)
  }
